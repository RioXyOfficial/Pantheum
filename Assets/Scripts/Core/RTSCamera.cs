...
namespace Pantheum.Core
{
    public class RTSCamera : MonoBehaviour
    {
        ...
        [SerializeField] private float _edgeScrollThickness = 20f;
        ...
        private void HandleKeyboardPan()
        {
            ...
            if (kb.upArrowKey.isPressed)    dir += GetForwardFlat();
            if (kb.downArrowKey.isPressed)  dir -= GetForwardFlat();
            if (kb.rightArrowKey.isPressed) dir += transform.right;
            if (kb.leftArrowKey.isPressed)  dir -= transform.right;
        ...
        private void HandleMiddleMousePan()
        {
            ...
            MovePosition((transform.right * delta.x + GetForwardFlat() * delta.y) * (_panSpeed * 0.02f));
        }
